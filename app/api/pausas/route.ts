...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ mensaje: "Pausa criada com sucesso", pausa }, { status: 201 });
}
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "O id da pausa é obrigatório" }, { status: 400 });
  ...
  if (!pausa || pausa.empleado.negocioId !== negocio!.id) {
